...
const folder=path.join(process.cwd(),'data');mkdirSync(folder,{recursive:true});
const db=new DatabaseSync(path.join(folder,'grandeur.sqlite'));db.exec('PRAGMA journal_mode=WAL; CREATE TABLE IF NOT EXISTS workspace (id INTEGER PRIMARY KEY, body TEXT NOT NULL)');
