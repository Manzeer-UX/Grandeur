...
function normalizeCaseLabels(value){
  if(typeof value==='string')return value.replace(/\bBottles\b/g,'Cases').replace(/\bBottle\b/g,'Case').replace(/\bbottles\b/g,'cases').replace(/\bbottle\b/g,'case');
  if(Array.isArray(value)){for(let i=0;i<value.length;i++)value[i]=normalizeCaseLabels(value[i]);return value}
  if(value&&typeof value==='object'){for(const key of Object.keys(value))value[key]=normalizeCaseLabels(value[key]);}
  return value;
}
...
export function read(){const state=normalizeCaseLabels(JSON.parse(db.prepare('SELECT body FROM workspace WHERE id=1').get().body));for(const [key,value] of Object.entries(prototypeDefaults))if(state[key]===undefined)state[key]=structuredClone(value);for(const connection of state.connections||[]){if(!connection.dataFlow)connection.dataFlow='Stock updates';if(!connection.method)connection.method=connection.endpoint?'API':'CSV / Excel upload';}if(!state.shipments.length&&state.orders?.length){const candidates=state.orders.filter(order=>order.warehouse&&order.distributor).slice(0,4);state.shipments=candidates.map((order,index)=>({id:`sample-shipment-${index+1}`,reference:`SHP-2026-${String(index+1).padStart(3,'0')}`,order:order.id,warehouse:order.warehouse,distributor:order.distributor,dispatchDate:'2026-09-07',deliveryDate:`2026-09-${String(9+index).padStart(2,'0')}`,status:['Created','Loading','In transit','Verified'][index]||'Created',initiatedBy:'Logistics Coordinator · Demo',verifiedBy:index===3?'Distributor Receiver · Demo':'',notes:'Static shipment record. No carrier connection is active.',synthetic:true}));}return state}
