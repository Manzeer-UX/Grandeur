...
export function stockByProduct(s){return s.products.map(p=>{const positions=s.distributors.flatMap(d=>(d.positions||[]).filter(r=>r.product===p.id));const warehouse=s.batches.filter(b=>b.product===p.id).reduce((n,b)=>n+Number(b.quantity),0);const held=positions.reduce((n,r)=>n+Number(r.stock),0);return {id:p.id,name:p.name,sku:p.sku||'Not assigned',size:p.size||'—',warehouse,distributor:held,total:warehouse+held,sold:positions.reduce((n,r)=>n+Number(r.sold),0),color:p.color||'#527ec4'};})}
